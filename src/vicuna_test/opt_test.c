...
static int8_t array[16] __attribute__ ((aligned (4))) = {
...
{
    dbg_printf(DEBUG_LEVEL_0, "Starting vicuna_opt_tests...\r\n");

    for (int i = 0; i < sizeof(test_funcs) / sizeof(void *); i++)
    {
        int ret = test_funcs[i]();
        if (ret == 0)
            dbg_printf(DEBUG_LEVEL_0, "[PASS] %s\r\n", test_names[i]);
        else
            dbg_printf(DEBUG_LEVEL_0, "[FAIL] %s function returned %d\r\n", test_names[i], ret);
    }
